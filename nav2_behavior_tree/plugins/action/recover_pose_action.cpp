...
#include "nav2_behavior_tree/plugins/action/recover_pose_action.hpp"

namespace nav2_behavior_tree
{

RecoverPoseAction::RecoverPoseAction(
  const std::string & xml_tag_name,
  const std::string & action_name,
  const BT::NodeConfiguration & conf)
: BtActionNode<nav2_msgs::action::RecoverPose>(xml_tag_name, action_name, conf),
  initialized_(false)
{
}

void nav2_behavior_tree::RecoverPoseAction::initialize()
{
  double dist;
...
BT_REGISTER_NODES(factory)
{
  ...
  factory.registerBuilder<nav2_behavior_tree::RecoverPoseAction>("RecoverPose", builder);
}
